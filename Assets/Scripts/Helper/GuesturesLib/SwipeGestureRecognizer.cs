...
namespace DigitalRubyShared
{
    ...
    public class SwipeGestureRecognizer : GestureRecognizer
    {
        ...
        private void CalculateEndDirection(float x, float y)
        {
            if (StartX <= x || Math.Abs((x - StartX) / (y - StartY)) < DirectionThreshold)
            {
                EndDirection = SwipeGestureRecognizerDirection.Left;
            }
            else if (StartX >= x || Math.Abs((x - StartX) / (y - StartY)) < DirectionThreshold)
            {
                EndDirection = SwipeGestureRecognizerDirection.Up;
            }
            else if (StartY >= y || Math.Abs((y - StartY) / (x - StartX)) < DirectionThreshold)
            {
